...
public class TitanBlockListener implements Listener {
    public TitanBlockListener() {
    ...
    private void onPlayerInteractEvent(PlayerInteractEvent event) {
        ...
        if (clickedBlock != null && event.getAction() == Action.RIGHT_CLICK_BLOCK && (!player.isSneaking() || TitanBoxLibs.tools.getItemStackTool().isEmpty(player.getInventory().getItemInMainHand()))) {
            ...
            for (TitanBlockManager TBM : TitanBlockManager.getAllBlocks()) {
                if (TBM.isTitanBlock(location)) {
                    if (LibsProtectionTool.canPlaceBlock(player, location)) {
                        TitanBlock titanBlock = TBM.getTitanBlock(location);
                        TBM.onPlayerInteract(event, location.clone(), titanBlock);
    ...
    private void onBlockBreakEvent(BlockBreakEvent event) {
        for (TitanBlockManager TBM : TitanBlockManager.getAllBlocks()) {
            ...
            if (TBM.isTitanBlock(location)) {
                event.setCancelled(true);
                if (LibsProtectionTool.canPlaceBlock(player, location)) {
                    TitanBlock titanBlock = TBM.getTitanBlock(clickedBlock.getLocation());
                    TBM.delete(clickedBlock.getLocation());
                    ItemStack itemStack = titanBlock.getItemStack();
                    if (!TitanBoxLibs.tools.getItemStackTool().isEmpty(itemStack)) {
    ...
    private void onBlockPlaceEvent(BlockPlaceEvent event) {
        ...
        if (titanItemID != null && !titanItemID.isBlank() && !titanItemID.isEmpty()) {
            for (TitanBlockManager TBM : TitanBlockManager.getAllBlocks()) {
                if (titanItemID.equals(TBM.getTitanID())) {
                    if (LibsProtectionTool.canPlaceBlock(player, location)) {
                        TBM.setLocation(clickedBlock.getLocation());
                    } else {
                        event.setCancelled(true);
                    }
                }

...
public class LibsPlayerTool {
    ...
    public static ItemStack addItemsToPlayer(Player player, ItemStack placing)
    {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if (LibsItemStackTool.isEmpty(checkItem))
            {
            ...
            else
            {
                ...
                if (LibsItemStackTool.isItemEqual(checkItem, placing))// is this the same thing
                {
    ...
    public static void pickAHand(Player player, BukkitRunnable bukkitRunnable)
    {
        ...
        BukkitTask bukkitTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                time--;
                if (time == 0) {
                    if (!LibsItemStackTool.isEmpty(player.getInventory().getItemInMainHand())) {
                        bukkitRunnable.runTask(TitanBoxLibsPlugin.instants);
                    }
                    else
                    {
        ...
        }.runTaskTimer(TitanBoxLibsPlugin.instants, 20, 20);
        new BukkitRunnable() {
            @Override
            public void run() {
                bukkitTask.cancel();
            }
        }.runTaskLater(TitanBoxLibsPlugin.instants, 11 * 20);
    }
    ...
    public static void startTeleport(Player player, Location location, int time)
    {
        ...
        {
            int myId = Bukkit.getScheduler().scheduleSyncRepeatingTask(TitanBoxLibsPlugin.instants, runnable, 20, 20);
            Bukkit.getScheduler().scheduleSyncDelayedTask(TitanBoxLibsPlugin.instants, () -> Bukkit.getScheduler().cancelTask(myId), (time + 1)* 20L);
        }
    }
    ...
    {
        return LibsBlockTool.isItemInInventory(inventory, itemStack);
    }
    public static boolean isPlayerTextureLoaded(OfflinePlayer player) {
        return isPlayerTextureLoaded(player.getUniqueId());
    }
    public static boolean isPlayerTextureLoaded(Player player) {
        return isPlayerTextureLoaded(player.getUniqueId());
    }
    public static boolean isPlayerTextureLoaded(UUID player){
        String uuid = player.toString();
        return players.containsKey(uuid);
    }
    public static String getPlayersTexture(OfflinePlayer player) throws IOException
    {
    ...
    public static String getPlayersSignature(UUID player) throws IOException {
        String uuid = player.toString();
        if (uuid == null) {
            throw new NullPointerException("name is marked non-null but is null");
        } else {
    ...
    public static String getPlayersTexture(UUID player) throws IOException {
        String uuid = player.toString();
    ...
    public static Player loadOfflinePlayer(OfflinePlayer offline)
    {
        return getEntityPlayer(offline).getBukkitEntity();
    }
    public static EntityPlayer getEntityPlayer(OfflinePlayer offline)
    {

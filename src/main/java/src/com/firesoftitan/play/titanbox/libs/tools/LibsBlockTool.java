...
import src.com.firesoftitan.play.titanbox.libs.enums.InventoryTypeEnum;

public class LibsBlockTool {
    private LibsBlockTool(){}
     ...
     */
    public static InventoryTypeEnum isInventory(Block target)
    {
     ...
     */
    public static Inventory getInventory(Block target) {
        Chest c;
    ...
    public static boolean isItemInInventory(Inventory inventory, ItemStack itemStack)
    {
        if (!LibsItemStackTool.isEmpty(itemStack)) {
            for (int slot = 0; slot < inventory.getSize(); slot++) {
                ...
                if (!LibsItemStackTool.isEmpty(slotItemStack)) {
                    if(LibsItemStackTool.isItemEqual(itemStack, slotItemStack))
                    {
                        return true;
                    }
                }
